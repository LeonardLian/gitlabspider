...
def finalDataChoose(percent):
    file_info = open("../data/esr_open-adventure_final.json", "rb")
    info_data = json.load(file_info)
    length = int(math.floor(len(info_data) * percent * 0.01))

    info_dataset = pd.DataFrame(info_data, columns=["changed_file_num", "average_filenum_per_commit_after_last_build",
                                                "commit_description_length", "time_interval_after_last_build",
                                                "time_interval_after_last_failure", "additions_num_after_last_build",
                                                "deletions_num_after_last_build", "commit_num_after_last_build",
                                                "last_build_result", "changed_java_file_num_after_last_build",
                                                "changed_config_file_num_after_last_build", "src_config_file_num",
                                                "same_committer", "committer_history", "committer_failure_rate_recent",
                                                "project_history", "failure_rate_last_five", "gaussian_threat_from_histroy",
                                                "total_commit_before", "hour_time", "weekday", "month_day",
                                                "developer_count", "collision_file_num", "collision_people_num", "build_result"
                                                ])
    # info_dataset = pd.DataFrame(info_data, columns=['changed_code_lines', 'changed_file_num', 'java_num', 'config_num',
